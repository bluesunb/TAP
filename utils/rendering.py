...
def save_video(file_name, frames, fps=30):
    file_name += '.mp4'
    imageio.mimsave(file_name, frames, fps=fps)
    print(f'Saved video to {file_name}')


def save_videos(file_name, *frames, **kwargs):
    video_frames = np.concatenate(frames, axis=2)
    save_video(file_name, video_frames, **kwargs)


def make_renderer(env_name):
    render_str = f'renderer_{env_name}'
    # render_class = getattr(sys.modules[__name__], render_str)
    if 'antmaze' in env_name:
        render_class = AntMazeRenderer
    elif 'kitchen' in env_name:
        render_class = KitchenRenderer
    elif 'maze2d' in env_name:
        render_class = Maze2dRenderer
    else:
        render_class = Renderer
    ...
    return render_class(env_name, observation_dim=observation.size)
