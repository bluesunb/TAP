...
from latent_planner.datasets.d4rl_utils import load_environment, qlearning_dataset_with_timeouts
...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config: DatasetConfig):
        ...
        self.env = env = load_environment(config.env) if type(config.env) is str else config.env
        self.sequence_length = config.seq_len
        self.step = config.step
        self.max_path_length = config.max_path_length
        self.device = config.device
        self.disable_goal = config.disable_goal
        
        print(f'[ datasets/sequence ] Loading...', end=' ', flush=True)
        if 'MineRL' in env.name:
            raise ValueError()
        dataset = qlearning_dataset_with_timeouts(env.unwrapped, terminate_on_end=True, disable_goal=config.disable_goal)
        print('✓')
        ...
        terminals_float = dataset['terminals_float']

        self.normalize_sa = config.normalize_sa
        ...
        if config.termination_penalty is not None:
            terminal_mask = terminals_float.squeeze()
            self.rewards_raw[terminal_mask] = config.termination_penalty

        # segment
        print(f'[ datasets/sequence ] Segmenting...', end=' ', flush=True)
        self.joined_segmented, self.termination_flags, self.path_lengths = \
            segment(self.joined_raw, terminals, config.max_path_length)
        self.rewards_segmented, *_ = segment(self.rewards_raw, terminals, config.max_path_length)
        print('✓')

        self.discount = config.discount
        self.discounts = (config.discount ** np.arange(self.max_path_length))[:,None]

        # [ n_paths x max_path_length x 1 ]
        self.values_segmented = np.zeros(self.rewards_segmented.shape, dtype=np.float32)

        for t in range(config.max_path_length):
            # [ n_paths x 1 ]
        ...
        self.train_portion = config.train_portion
        self.test_portion = 1.0 - config.train_portion
        # get valid indices
        indices = []
        test_indices = []
        for path_ind, length in enumerate(self.path_lengths):
            ...
            for i in range(end):
                if i < split:
                    indices.append((path_ind, i, i+config.seq_len))
                else:
                    test_indices.append((path_ind, i, i+config.seq_len))
        ...
        self.joined_segmented = np.concatenate([
            ...
            np.zeros((n_trajectories, config.seq_len - 1, joined_dim), dtype=np.float32),
        ], axis=1)
        ...
        self.termination_flags = np.concatenate([
            ...
            np.ones((n_trajectories, config.seq_len - 1), dtype=bool),
        ], axis=1)
    ...
    def denormalize_rewards(self, rewards):
        if (not self.normalize_sa) or (not self.normalize_reward):
            return rewards
    ...
    def denormalize_values(self, values):
        # if (not self.normalize_sa) or (not self.normalize_reward):
        if not (self.normalize_sa and self.normalize_reward):
                return values
    ...
    def __getitem__(self, idx):
        ...
        # flatten everything
        X = joined[:-1]     # data_0 ... data_{T-1}
        Y = joined[1:]      # data_1 ... data_{T}
        mask = mask[:-1]
        terminal = terminal[:-1]
